...
def histogram_predict(belief, left_encoder_ticks, right_encoder_ticks, grid_spec, robot_spec, cov_mask):
    ...
    delta_theta_left = 2 * np.pi/robot_spec['encoder_resolution'] * left_encoder_ticks
    delta_theta_right = 2 *np.pi/robot_spec['encoder_resolution'] * right_encoder_ticks
    
    d_left = delta_theta_left * robot_spec['wheel_radius']
    d_right = delta_theta_right * robot_spec['wheel_radius']
    d_A = (d_left + d_right)/2
    d_phi = (d_right - d_left)/robot_spec['wheel_baseline']
    ...
    # v = d_A * np.sin(phi_max)#0.0  # replace this with a function that uses the encoder
    # w = d_phi #0.0  # replace this with a function that uses the encoder
    ...
    for i in range(belief.shape[0]):
        for j in range(belief.shape[1]):
            ...
            if belief[i, j] > 0:
                ...
                i_new = np.max(np.where(d_t[i,j] >= grid_spec['d'][:,0])) 
                j_new = np.max(np.where(phi_t[i,j] >= grid_spec['phi'][0,:])) 
...
def generate_measurement_likelihood(segments, road_spec, grid_spec):
    ...
    for segment in segments:
        ...
        i = np.max(np.where(d_i >= grid_spec['d'][:,0])) 
        j = np.max(np.where(phi_i >= grid_spec['phi'][0,:])) 
